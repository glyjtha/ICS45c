...
class Coins {
public:
	//constucts 'this' Coins object so it has the number of each coin given in the
	...
	//Returns true only if 'this' Coins object has enough of each type of coint to cover a
	//a possible extraction of the specific coins in 'coins'
	bool has_exact_change_for_coins(const Coins& coins) const;

	//Removes from 'this' Coins object the specific number of each type of coin given in
	//'coins and returns a 'Coins' object representing the coins that were extracted
	//For any invalid extraction request, return an empty Coins objet (i.e. 0 of all coins)
	...
	// Always prints plural even for 1, e.g. "1 nickels". This is the method by the output
	// stream inserter below.
	void print(std::ostream& out) const;

	// Allows us to compare Coins objects b equality. You *don't* have to implement anything here,
...
//coins_required_for_cents() can use math or while loops to decide how to satisfry the need for
//amount_in_cents. First, take as many quarters, then as many dimes, etx. E.g.,
...
//Ask for coins on output stream 'out' and reads entered values on input stream 'in', then finally
...
//extractions, and printing the balance. For precise requirements see hw writeup
